...
import { Leaf, CheckCircle2, MailWarning } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      if (result.unverified) {
        ...
        toast.error(result.error.includes("Invalid") ? "Invalid email or password" : result.error);
      }
    } else {
        ...
        <div className="w-full max-w-sm">
          ...
          {unverifiedEmail && (
            <div className="mb-5 flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
              <MailWarning className="h-5 w-5 shrink-0 mt-0.5 text-amber-500" />
              <div>
